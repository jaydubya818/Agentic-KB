...
echo "→ Running tests..."
node --test tests/foundry-integration.test.mjs
...
feat(foundry): install Foundry slash commands + 2-source compile gate
...
- tests/foundry-integration.test.mjs      28 tests, node --test, no deps
- wiki/candidates.md, wiki/_meta/         gate scaffold
...
All 28 tests pass. Gate smoke-tested against real wiki: 29 promote, 108 defer.
...
if gh pr view feat/foundry-integration >/dev/null 2>&1; then
  ...
  gh pr create --title "Foundry: slash commands + 2-source compile gate" --body "$(cat <<'EOF'
## Summary
- 4 slash commands (`/foundry-ingest`, `/foundry-compile`, `/foundry-ask`, `/foundry-lint`) as UX layer over existing `kb` CLI — single source of truth, no parallel vault
...
## Test plan
- [x] 28/28 unit tests pass: `node --test tests/foundry-integration.test.mjs`
...
- [ ] Smoke-test `/foundry-compile --plan` matches pre-merge baseline
...
- Zero `kb` CLI changes — gate calls existing `node cli/kb.js compile`
