...
A P0–P3 enterprise roadmap for scaling Agentic-KB from a personal tool to a team/enterprise platform, grounded in the [[andrej-karpathy]] LLM-Wiki pattern. Core insight: "Every business has a raw/ directory. Nobody's ever compiled it. That's the product."
...
- **[[openai]]-compatible chat completion endpoint** — works with LangChain, [[framework-autogen]], [[framework-crewai]], Cursor
...
Three-layer design: INGEST (raw docs, webhooks, GitHub, Slack, Notion) → COMPILE (LLM builds wiki, updates cross-references, schema-guided) → QUERY (semantic + keyword, [[openai]]-compat API, [[mcp-ecosystem]] tools, CLI). Storage: `raw/` (immutable), `wiki/` (compiled), `logs/` (audit JSONL), `embeddings/` (SQLite vectors).
