...
Chain-of-thought prompting elicits step-by-step reasoning from LLMs before producing final answers. It consistently improves accuracy on multi-step reasoning tasks. The tradeoff is token cost and latency. Extended thinking (Claude's native [[chain-of-thought]]) runs reasoning in a separate block invisible to users but available to the model. Use [[chain-of-thought]] when reasoning steps matter; skip it for lookup or simple generation tasks.

---

## Definition

Chain-of-thought ([[chain-of-thought]]) is a prompting technique that encourages an LLM to produce intermediate reasoning steps before arriving at a final answer. Rather than jumping directly to output, the model "thinks out loud," which both improves accuracy and makes reasoning inspectable.
...
Few-shot [[chain-of-thought]] outperforms zero-shot [[chain-of-thought]] for complex, domain-specific problems where the reasoning pattern isn't obvious. The demonstrations show the model what "good reasoning" looks like in this domain.

### Extended Thinking (Claude)

Claude's native implementation of internal [[chain-of-thought]]. The model reasons in a separate `thinking` block before producing its response. Key properties:
...
### Structured [[chain-of-thought]] Formats
...
Structured [[chain-of-thought]] enables automated parsing of reasoning steps, not just the final answer. Useful in evaluation pipelines and for generating reasoning traces for fine-tuning.
...
- Simple factual lookup ("What is the capital of France?") — [[chain-of-thought]] adds tokens, no accuracy gain
...
- High-throughput, latency-sensitive applications — every [[chain-of-thought]] token costs time and money
- Tasks where the reasoning trace itself can mislead the model (rare but documented)

### Cost and Latency Impact

[[chain-of-thought]] tokens are output tokens — they're expensive and slow. A 5x improvement in reasoning accuracy might come with 5x the tokens. Budget accordingly:
...
|---------|-----------|----------|
| Zero-shot [[chain-of-thought]] | "Think step by step" | General reasoning, no examples available |
| Few-shot [[chain-of-thought]] | Demonstrations + target | Domain-specific reasoning |
...
- **Confident wrong reasoning**: The model produces a plausible-sounding but incorrect reasoning chain and then a wrong answer. [[chain-of-thought]] doesn't guarantee correctness; it just makes errors visible.
...
- [[concepts/agent-loops]] — [[chain-of-thought]] within the reasoning step of each loop iteration
- [[concepts/self-critique]] — using [[chain-of-thought]] to critique and revise output
- [[concepts/cost-optimization]] — balancing [[chain-of-thought]] quality gain against token cost
