...
1. **Requirements stability** — shifting → GSD, locked → [[framework-bmad]]
2. **Reversibility of mistakes** — costly to undo → [[framework-superpowers]]
...
2. Onboard **[[framework-superpowers]]** for the stability layer (auth, payments, agents)
3. Use **[[framework-bmad]]** for any future spec-locked modules (e.g., a CRM add-on)
...
[[framework-superpowers]] is described as having "two-stage review" (spec review + quality review) at each implementation step. This is significantly more process overhead than GSD's single-stage verification.

## [[framework-bmad]] Details
...
- Party Mode: Multiple [[framework-bmad]] agents (PM, Architect, UX, Dev) debate decisions simultaneously rather than sequentially

[[framework-bmad]] has the heaviest upfront investment in planning — the spec is fully validated before any implementation begins. This makes it unsuitable for exploratory work.
...
- `claude-opus-4-6`: complex architecture, security audits ([[framework-superpowers]]/[[framework-bmad]]-level analysis)

The model tier selection mirrors the framework tier selection — more process ([[framework-bmad]]/[[framework-superpowers]]) correlates with more capable (and expensive) models for critical decisions.
