...
**Vs. [[framework-claude-code]] / raw API**: Assistants API manages thread persistence server-side (no need to maintain message history locally) but at the cost of control — you can't directly inspect or modify the thread's internal state. Useful for simple persistent chat agents; limiting for complex multi-agent orchestration.
...
## Realtime API
[[openai]]'s streaming audio input + output API — enables real-time voice conversations with the model (speech-to-speech, not speech-to-text-to-speech). WebSocket-based. Powers real-time voice agents. Claude has no direct equivalent API endpoint ([[framework-claude-code]] can process audio files, but not streaming voice). Key for: voice assistants, real-time interview coaching, live interpretation.
...
| Agent runtime | Assistants API (server-managed) | [[framework-claude-code]] (CLI, user-run) |
...
| Coding assistant | Codex in IDEs, no CLI | [[framework-claude-code]] (full CLI agent) |

For Jay's stack specifically: [[framework-claude-code]]'s superiority as a development environment and [[anthropic]]'s larger context windows are the primary reasons for being [[anthropic]]-first. [[openai]] is used when needed for multimodal audio (Realtime API), image generation (DALL-E), or when a client/tool mandates GPT.
