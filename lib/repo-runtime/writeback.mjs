...
export function closeRepoTask(kbRoot, repoName, contract, payload) {
  // Dry-run short-circuit — compute the plan, return it, touch nothing.
  // Mirrors closeTask in ../agent-runtime/writeback.mjs. Without this branch
  // `dryRun` was simply an unrecognised payload key here, so a caller that
  // asked the repo twin for a plan got a real, committed close: progress.md
  // appended, hot.md written, bus items published — and nothing in the return
  // value said so, because `dryRun` was absent and `committed` was true.
  if (payload && payload.dryRun === true) {
    // `ok` must track the plan — a rejected plan is not a successful call.
    const plan = dryRunCloseRepoTask(kbRoot, repoName, contract, payload)
