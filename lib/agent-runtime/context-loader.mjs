...
export function loadAgentContext(kbRoot, contract, vars = {}) {
  ...
  // Sort by priority. If contract has priority_order, use it as the primary sort key
  // so named buckets (profile, hot, project, subscriptions, learned, standards) are
  // honoured before falling back to the numeric priority field.
  const priorityOrder = contract.context_policy?.priority_order || []
  function bucketRank(item) {
    // Determine the named bucket for a candidate based on its reason string or class.
    if (item.reason.startsWith('bus:')) {
      const i = priorityOrder.indexOf('subscriptions')
      return i >= 0 ? i : 999
    }
    // class-based
    const cls = item.class || item.reason.match(/class=(\w+)/)?.[1] || ''
    const nameMap = { profile: 'profile', hot: 'hot', working: 'hot', learned: 'learned', rewrite: 'learned', bus: 'subscriptions' }
    const named = nameMap[cls] || cls
    const i = priorityOrder.indexOf(named)
    if (i >= 0) return i
    // path-based heuristics
    if (item.reason.includes('project')) {
      const pi = priorityOrder.indexOf('project')
      return pi >= 0 ? pi : 999
    }
    if (item.reason.includes('standards')) {
      const si = priorityOrder.indexOf('standards')
      return si >= 0 ? si : 999
    }
    return 999
  }
  ...
  candidates.sort((a, b) => {
    if (priorityOrder.length > 0) {
      const ra = bucketRank(a), rb = bucketRank(b)
