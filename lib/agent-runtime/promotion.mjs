...
export function promoteDiscovery(kbRoot, opts = {}) {
  ...
  const promoted = serializeFrontmatter({
    ...
    // Carry the qualifiers the scorer read. Without these the promoted page
    // asserts every claim with equal weight: a `confidence: low` observation
    // backed by nothing and a `confidence: high` one backed by nine pieces of
    // evidence produced byte-identical frontmatter, so an uncertain claim read
    // as unqualified fact and the citations that justified it were reachable
    // only by following `source_path` back to the bus item. Defaults match the
    // ones scorePromotion applies above, so the page records what was scored.
    confidence: item.meta.confidence || 'medium',
    related_sources: item.meta.related_sources || [],
    evidence_count: item.meta.evidence_count || 0,
