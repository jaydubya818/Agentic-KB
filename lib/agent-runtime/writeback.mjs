...
export function closeTask(kbRoot, contract, payload) {
  ...
  // 2. Commit all (files + bus) in one pass. If any write throws, restore the
  // pre-close filesystem state and return a rolled-back result.
  const rollbacks = []
  try {
  ...
  for (const write of trace.writes_committed) {
    appendAudit(kbRoot, { op: 'agent-write', agent_id: contract.agent_id, path: write.path, kind: write.op })
  }
  ...
  if (sealedTask) {
    appendAudit(kbRoot, { op: 'agent-seal-task', agent_id: contract.agent_id, task_id: sealedTask.taskId })
  }
